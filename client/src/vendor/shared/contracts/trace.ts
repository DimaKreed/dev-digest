...
export const PromptAssembly = z.object({
  ...
  /** Tokens the skills block added to the prompt; null when no skills were
      injected. `nullish` for the same reason as `RunStats.cost_usd` — this
      object round-trips through the `run_traces.trace` jsonb, and traces
      written before this key existed simply omit it. */
