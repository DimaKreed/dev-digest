...
- [Onboarding Generator](./onboarding-generator.md) — why the tour is a blocking POST rather
  ...
  stay apart.
- [Eval Pipeline](./eval-pipeline.md) — why scoring reaches no model, why precision counts
  only what a `must_not_flag` case forbids, why a batch is rows sharing a `batch_id` rather
  than a table, why an errored case is excluded from every metric instead of scored zero, and
  the runbook for the prompt-ablation experiment on PR #485.
